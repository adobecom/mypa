...
Every tool call is capped by `callToolTimed`'s `withTimeout` wrapper — 30 s by default, overridden per server via a small map (Slack is 150 s, since its `xoxp`-token fallback path is one API call per channel). The override is also passed as `client.callTool()`'s own `RequestOptions.timeout`, not just raced via `withTimeout` — the MCP SDK's `Client` has an internal 60 s default request timeout that fires independently and will cut a call off at 60 s regardless of a larger `withTimeout` value unless told otherwise. `connectServer` also does one Slack-specific env injection: `SLACK_MCP_USERS_CACHE`/`SLACK_MCP_CHANNELS_CACHE` paths under `~/.mypa`, required for `slack-mcp-server` to resolve names and for `channels_list` to work at all. See [mcp-and-oauth.md](mcp-and-oauth.md) for the full rationale.
...
- 2026-07-23 — **Fix Slack routines always timing out with an opaque "Agent timed out" (`mcp.ts`, `agent.ts`).** The "Summarize Slack & Flag Missed Convos" routine never once succeeded: `~/.mypa/mypa.log` showed `slack::conversations_unreads` called 44 times, every one hitting the flat 30 s `callToolTimed` cap, until the routine's 300 s wall-clock timeout fired and discarded everything — surfacing only `Error: Agent timed out` with no indication which tool or server was actually the problem. Root cause is `slack-mcp-server`'s documented behavior with an `xoxp` user token: `conversations_unreads` falls back to one Slack API call per channel and is slow on large workspaces, and without `SLACK_MCP_USERS_CACHE`/`SLACK_MCP_CHANNELS_CACHE` set, `channels_list` doesn't work at all. Fixes: `connectServer` now injects those two cache-file env vars for the `slack` server when unset (confirmed live: a 22 MB user cache built itself on first connect, against a workspace clearly too large for the old per-call flow to survive); `callToolTimed` now resolves its timeout from a per-server override map (Slack: 150 s, default: 30 s unchanged), passed through to `client.callTool()`'s own `RequestOptions.timeout` — a first pass that only widened the outer `withTimeout` race was confirmed live to be a no-op, since the MCP SDK's own internal 60 s default request timeout fired first regardless, surfacing `MCP error -32001: Request timed out`; and `runRoutineAgentOnce` now short-circuits after the same `server::tool` fails twice rather than grinding to the full 300 s, returning whatever was already gathered as a partial result (mirroring the existing `maxTurns`/`is_error` partial-result path) instead of throwing a bare `Agent timed out` — so a real failure like `callTool slack::conversations_unreads timed out after 150s` now reaches `routine_runs.error`. Confirmed live: the routine's fallback path also attempted a user-group membership tool (list/join/leave) that unattended runs correctly deny as a write tool — the routine prompt was tightened to steer its fallback to only genuinely read-only tools (`channels_list`, `conversations_history`, `conversations_search_messages`). See [mcp-and-oauth.md](mcp-and-oauth.md#changelog) and [claude-integration.md](claude-integration.md#changelog).
