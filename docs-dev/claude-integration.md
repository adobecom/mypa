...
`runRoutineAgentOnce` also tracks failure counts per `server::tool` key. A tool that is fundamentally broken for this run (e.g. a Slack call that can't finish inside the MCP layer's own timeout on a given workspace) fails identically on every retry rather than eventually succeeding, so once the same tool has failed twice the run aborts (`AbortController.abort()`) instead of burning the rest of the 300 s budget on a predictable dead end. This abort is handled the same way as the timeout abort: if anything was gathered before aborting (`toolOutputs.length > 0 || failures.length > 0`), the partial `RoutineAgentResult` is returned rather than thrown, so the real per-tool failure message (e.g. `callTool slack::conversations_unreads timed out after 150s`) survives into `routines.ts`'s error summary. Only a run that aborts with literally nothing gathered still throws the bare `Error('Agent timed out')`.
...
- 2026-07-23 — **`runRoutineAgentOnce`: short-circuit on repeated tool failures, return partial results on abort instead of a bare "Agent timed out" (`agent.ts`).** A Slack routine calling `conversations_unreads` kept failing the same MCP-layer 30 s timeout on every retry, and the routine's flat 300 s run cap ate the full budget before aborting — discarding every already-recorded `failures[]` entry and surfacing only the opaque `Error('Agent timed out')`, with no indication which server/tool actually failed. Added a `failureCounts` map keyed by `${server}::${tool}`; once the same tool fails twice, the run aborts immediately via the same `AbortController` the timeout uses. The abort's `catch` now checks whether anything was gathered (`toolOutputs.length > 0 || failures.length > 0`) — timeout or repeated-failure — and if so returns the partial `RoutineAgentResult` rather than throwing, exactly like the existing `resultMsg.is_error`-with-partial-data path just above it. Only a run that aborts with nothing at all gathered still throws the bare timeout error. See [services.md](services.md#changelog) for the accompanying Slack MCP config fix that addresses why the tool was failing in the first place.
